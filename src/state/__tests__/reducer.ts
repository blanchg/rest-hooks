...
describe('reducer', () => {
  ...
  describe('rest-hooks/optimistic-update', () => {
    it('it should run inserts', () => {
      const id = 20;
      function makeOptimisticAction(url: string, items: string[]): OptimisticUpdateAction {
        return {
          type: 'rest-hooks/optimistic-update',
          payload: {
            [url]: ((result: string[] | undefined) => [...items, ...(result  || [])]) as any,
          }
        };
      }
      const iniState: any = {
      ...
      const newState = reducer(iniState, makeOptimisticAction(ArticleResource.listUrl(), ['10']));
      expect(newState.results[ArticleResource.listUrl()]).toStrictEqual(['10']);
      const newState2 = reducer(newState, makeOptimisticAction(ArticleResource.listUrl(), ['20', '25']));
      expect(newState2.results[ArticleResource.listUrl()]).toStrictEqual(['20', '25', '10']);
    });
  });
